...
exports.wrappableHandle = exports.isNil = exports.handleUpdateOne = exports.handleGetOneSafe = exports.handleGetOne = exports.handleGetMany = exports.handleDeleteOne = exports.handleCreateOne = exports.handle = exports.getMin = exports.getMax = exports.fetchJsonOrThrow = void 0;
...
function handleGetMany(config) {
    ...
    var executor = function (parsed, context) { return config.action(parsed.query, context, parsed.fieldRequest); };
    return wrappableHandle(__assign(__assign({}, config), { parser: parser, executor: executor }));
}
...
function handleGetOne(config) {
    ...
    var executor = function (parsed, context) { return config.action(parsed.params, context, parsed.fieldRequest); };
    return wrappableHandle(__assign(__assign({}, config), { parser: parser, executor: executor }));
}
...
function handleCreateOne(config) {
    ...
    var executor = function (parsed, context) { return config.action(parsed.body, context, parsed.fieldRequest); };
    return wrappableHandle(__assign(__assign({}, config), { parser: parser, executor: executor, code: 201 }));
}
...
function handleUpdateOne(config) {
    ...
    var executor = function (parsed, context) { return config.action(parsed.params, parsed.body, context, parsed.fieldRequest); };
    return wrappableHandle(__assign(__assign({}, config), { parser: parser, executor: executor }));
}
...
function wrappableHandle(config) {
    var parserWrapper = config.parserWrapper, executorWrapper = config.executorWrapper, parserRaw = config.parser, executorRaw = config.executor, rest = __rest(config, ["parserWrapper", "executorWrapper", "parser", "executor"]);
    var parser = parserWrapper === undefined ? parserRaw : parserWrapper(parserRaw);
    var executor = executorWrapper === undefined ? executorRaw : executorWrapper(executorRaw);
    return handle(__assign(__assign({}, rest), { parser: parser, executor: executor }));
}
exports.wrappableHandle = wrappableHandle;
// Note: request => authenticate => parse => execute => respond
function handle(config) {
    var _this = this;
    return function (request) { return __awaiter(_this, void 0, void 0, function () {
        var data, _a, _b, _c, error_2;
        return __generator(this, function (_d) {
            switch (_d.label) {
                ...
                case 2:
                    _a.context = _d.sent();
                    _b = data;
                    return [4 /*yield*/, config.parser(request, data.context)];
                case 3:
                    _b.parsed = _d.sent();
                    if (!(config.validator !== undefined)) return [3 /*break*/, 5];
                    return [4 /*yield*/, config.validator(data.parsed, data.context, config.options)];
                case 4:
                    _d.sent();
                    _d.label = 5;
                case 5:
                    _c = data;
                    return [4 /*yield*/, config.executor(data.parsed, data.context)];
                case 6:
                    _c.result = _d.sent();
                    return [2 /*return*/, respond(data.result, config.code)];
                case 7:
                    error_2 = _d.sent();
                    if (!(config.errorHandler === undefined)) return [3 /*break*/, 8];
                    throw error_2;
                case 8: return [4 /*yield*/, config.errorHandler(error_2, data)];
